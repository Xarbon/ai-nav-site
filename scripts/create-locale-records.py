...
"""
创建 locale 架构的记录：
- zh 记录：locale='zh'，slug='chatgpt'
- en 记录：locale='en'，slug='chatgpt-en'
"""
...
if en_count and en_count[0]['cnt'] > 0:
    print(f'\n已有 {en_count[0]["cnt"]} 个 en 记录，先删除...')
    d1_exec("DELETE FROM tools WHERE locale = 'en'")
    time.sleep(2)

print('\nStep 2: 为每个 zh 工具创建 en 记录...')
tools = d1_query("SELECT * FROM tools WHERE locale = 'zh'")
