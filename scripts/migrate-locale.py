...
迁移方案：
1. 添加 locale 列（默认 'zh'）
2. 为每个 zh 记录创建对应的 en 副本记录
3. en 记录使用 zh 的 _en 字段数据填入主字段
"""
...
print('Step 1: 添加 locale 列...')
d1_exec("ALTER TABLE tools ADD COLUMN locale TEXT DEFAULT 'zh'")
time.sleep(2)

# Verify
result = d1_query("SELECT COUNT(*) as cnt FROM tools WHERE locale = 'zh'")
print(f'  zh 记录数: {result[0]["cnt"] if result else 0}')

print('\nStep 2: 为每个 zh 工具创建 en 副本...')
# Get all zh tools
tools = d1_query("SELECT * FROM tools WHERE locale = 'zh' OR locale IS NULL")
print(f'  找到 {len(tools)} 个 zh 工具')

# Check if en records already exist
existing_en = d1_query("SELECT COUNT(*) as cnt FROM tools WHERE locale = 'en'")
en_count = existing_en[0]['cnt'] if existing_en else 0
print(f'  已有 {en_count} 个 en 记录')

if en_count > 0:
    print('  已有 en 记录，先删除旧的...')
    d1_exec("DELETE FROM tools WHERE locale = 'en'")
    time.sleep(2)

count = 0
for t in tools:
    # Create en record: use _en fields as main fields
    ...
    # Generate a unique id for en record
    import uuid
